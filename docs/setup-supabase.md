...
2. Open [`ops/apply-all.sql`](../ops/apply-all.sql) in a text editor, select
   **everything**, paste, press **Run**. It is every file in
   ...
   `0001_init.sql`, `0002_functions.sql` and `0003_control_bot.sql` — generated by
...
order, in three Runs. **Order matters**: 0002 creates the triggers, queue functions
and views that 0001's tables depend on, and 0003 adds
`app.telegram_session` — the table the control bot writes the login into — plus the
`bot.*` rows in `app.config`.
...
`app.telegram_session` from 0003) and 4 views. A smaller number means a migration
