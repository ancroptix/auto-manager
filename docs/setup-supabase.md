...
2. Open [`ops/apply-all.sql`](../ops/apply-all.sql) in a text editor, select
   **everything**, paste, press **Run**. It is every file in
   ...
   `0001_init.sql`, `0002_functions.sql`, `0003_control_bot.sql` and
   `0004_approved_captions.sql` — generated by `ops/build_apply_all.py`, and a test
   fails if it ever drifts from the directory.
...
(and refuses to overwrite a caption you have since edited).
...
`app.telegram_session` from 0003) and 4 views. 0004 adds no table — it adds one
column (`app.series.subtitle`) and three `app.config` rows. A smaller number means a migration
did not run; a bigger one means something else is sharing the schema.
