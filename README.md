...
| Discovery: the spare account's channels sorted by role and paired by name — the channel we can only read is the source, the one we post in is that series' destination, and one tap sets the show up (`/discover`, `app/discover.py`) | built and tested against fakes; rights are asked of Telegram per channel, it creates no channel, founds a missing series row only from a channel's own title and says so, refuses to strand a series' only source, and switches on its own only when `discover.auto` is on |
