...
def parse_episode(
    *,
    ...
    if require_hindi_audio and audio_kind in (AudioKind.SUBBED_ONLY, AudioKind.NON_HINDI_DUB):
        ...
        # file. It waits for a human, which is the only safe answer. There is no
        # exception for in-place captioning, because that mode captions a file and
        # still publishes it — the audience is the same, so the standard is too.
